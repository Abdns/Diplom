...
public class VolumeObjDataset  : ScriptableObject
{
        ...
        for (int x = 0; x < dimX; x++)
        {
            for (int y = 0; y < dimY; y++)
            {
                for (int z = 0; z < dimZ; z++)
                {
                    ...
                    int x1 = data[Mathf.Min(x + 1, dimX - 1) + y * dimX + z * (dimX * dimY)] - minValue;
                    int x2 = data[Mathf.Max(x - 1, 0) + y * dimX + z * (dimX * dimY)] - minValue;
                    int y1 = data[x + Mathf.Min(y + 1, dimY - 1) * dimX + z * (dimX * dimY)] - minValue;
                    int y2 = data[x + Mathf.Max(y - 1, 0) * dimX + z * (dimX * dimY)] - minValue;
                    int z1 = data[x + y * dimX + Mathf.Min(z + 1, dimZ - 1) * (dimX * dimY)] - minValue;
                    int z2 = data[x + y * dimX + Mathf.Max(z - 1, 0) * (dimX * dimY)] - minValue;
